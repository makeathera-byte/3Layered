...
export default function ActionFigurePage() {
  ...
  const loadProducts = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/products?category=action-figure');
  ...
  return (
    <section className="relative glass rounded-2xl p-4 sm:p-8">
      ...
      <p className="mt-2 text-sm sm:text-base text-moss">
        Detailed and collectible 3D-printed action figures for enthusiasts.
      </p>
      ...
      {loading ? (
        ...
        <div className="mt-8 text-center py-12">
          <p className="text-moss text-lg">
            No action figures available yet
          </p>
