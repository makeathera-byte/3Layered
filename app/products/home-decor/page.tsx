...
export default function HomeDecorPage() {
  ...
  const loadProducts = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/products?category=home-decor');
  ...
  return (
    <section className="relative glass rounded-2xl p-4 sm:p-8">
      ...
      <p className="mt-2 text-sm sm:text-base text-moss">
        Transform your space with our thoughtfully designed 3D-printed home décor pieces.
      </p>
      ...
      {loading ? (
        ...
        <div className="mt-8 text-center py-12">
          <p className="text-moss text-lg">
            No home décor products available yet
          </p>
